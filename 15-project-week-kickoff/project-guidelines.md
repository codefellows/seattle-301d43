...
_Why:_
> Any items in a `.gitignore` file will not be pushed to GitHub. This is useful for files such as `.DS_Store`, `.env`, and `node_modules`.

### <a id="environments">Environments</a>
Your project should use `dotenv` and `.env` files for management of local environment variables, and should NOT commit those files in any commit of your project.

_Why:_
> Your team will need to manage local environment variables to ensure a working development environment on each of your computers, and also need to manage config vars on Heroku for any API keys. If this information shows up anywhere in your commit history, it is visible/steal-able on GitHub.
...
- Clearly defined API endpoints with sample responses (for all JSON endpoints of your app)
- Clearly defined site endpoints with description and/or screenshot (for all HTML endpoints of your app)
...
In general, do not show code during the presentation unless the audience asks to see it. Have a code editor open just in case though. It's also appropriate to show a few lines of code in a slide deck, if you're particularly proud of a few lines or want to explain how some feature works.
...
- Good and proper use of CSS
  ...
  - Designed with a mobile-responsive approach, including at least one breakpoint
...
- Data is persisted in a SQL database
  ...
  - Ability to either update or delete records from the database (at least one!)
...
- Generally useful and functional
  ...
  - State is persisted between page reloads, using client-side and server-side persistance as appropriate
  - [Deployed](#deployment) live on the Internet via Heroku
  - Strongly encouraged: your site is at a custom domain. (We can help with this and can reimburse you for cheap domains!)
